...
console.log('seetting user color as ', window.userColor)
...
let App = {
  init(){
    ...
    this.pad = new Sketchpad(this.el, window.username, { line: { color: window.userColor } })
    ...
    this.padChannel.on('png_request', () => {
      console.log('png export was requested from this client')
    ...
    this.padChannel.on("stroke", ({user_id, stroke}) => {
      this.pad.putStroke(user_id, stroke, {color: stroke.color})
    })
